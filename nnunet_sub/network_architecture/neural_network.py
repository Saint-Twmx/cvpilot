...
from nnunet_sub.utilities.random_stuff import no_op
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_maybe_mirror_and_pred_3D(
        ...
        if hasattr(self, "trt_engine"):
            infer = self.trt_engine.__call__
        else:
            infer = self.__call__
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred = self.inference_apply_nonlin(infer(x))
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (4,))))
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (3,))))
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (4, 3))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (2,))))
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (4, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (3, 2))))
            ...
            if (
                ...
                pred = self.inference_apply_nonlin(infer(torch.flip(x, (4, 3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3, 2))

        if mult is not None:
            result_torch[:, :] *= mult

        ##### specific postprocess to inhibit cuspid over-segmentation #####
        result_torch[:, 7, :, :, :] /= 3
        return result_torch
