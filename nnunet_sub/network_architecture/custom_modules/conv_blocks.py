...
class ConvDropoutNormReLU(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, network_props):
        ...
        if network_props["norm_op"] is not None:
            self.norm = network_props["norm_op"](
                output_channels, **network_props["norm_op_kwargs"]
            )
        else:
            self.norm = Identity()
...
class StackedConvLayers(nn.Module):
    def __init__(
        ...
        self.convs = nn.Sequential(
            ConvDropoutNormReLU(
                input_channels, output_channels, kernel_size, network_props_first
            ),
            *[
                ConvDropoutNormReLU(
    ...
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        ...
        if props["dropout_op_kwargs"] is None and props["dropout_op_kwargs"] > 0:
            raise NotImplementedError(
                "ResidualBottleneckBlock does not yet support dropout!"
            )
...
class ResidualLayer(nn.Module):
    def __init__(
        ...
        self.convs = nn.Sequential(
            ...
            *[
                block(output_channels, output_channels, kernel_size, network_props)
