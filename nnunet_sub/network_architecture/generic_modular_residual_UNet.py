...
import numpy as np
import torch
from nnunet_sub.network_architecture.custom_modules.conv_blocks import (
    BasicResidualBlock,
    ResidualLayer,
)
from nnunet_sub.network_architecture.generic_UNet import Upsample
from nnunet_sub.network_architecture.generic_modular_UNet import (
...
class ResidualUNetEncoder(nn.Module):
    def __init__(
        ...
        self.initial_norm = props["norm_op"](
            base_num_features, **props["norm_op_kwargs"]
        )
...
class ResidualUNet(SegmentationNetwork):
    ...
    def compute_approx_vram_consumption(
        ...
        enc = ResidualUNetEncoder.compute_approx_vram_consumption(
            patch_size,
            base_num_features,
            max_num_features,
            num_modalities,
        ...
        dec = ResidualUNetDecoder.compute_approx_vram_consumption(
            patch_size,
            base_num_features,
            max_num_features,
            num_classes,
...
def find_3d_configuration():
    ...
    unet = FabiansUNet(
        ...
        num_classes,
        blocks_per_stage_decoder[: len(conv_op_kernel_sizes) - 1],
        False,
        False,
    ...
    print(
        FabiansUNet.compute_approx_vram_consumption(
            patch_size,
            initial_num_features,
            max_num_features,
            num_modalities,
