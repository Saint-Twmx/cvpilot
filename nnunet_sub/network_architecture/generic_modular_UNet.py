...
import numpy as np
import torch
from nnunet_sub.network_architecture.custom_modules.conv_blocks import StackedConvLayers
from nnunet_sub.network_architecture.generic_UNet import Upsample
...
def get_default_network_config(
    dim=2, dropout_p=None, nonlin="LeakyReLU", norm_type="bn"
):
    """
    ...
    props["conv_op_kwargs"] = {
        ...
        "dilation": 1,
        "bias": True,
    }  # kernel size will be set by network!
...
class PlainConvUNetEncoder(nn.Module):
    def __init__(
        ...
        for stage in range(num_stages):
            current_output_features = min(
                int(base_num_features * feat_map_mul_on_downscale**stage),
                max_num_features,
            )
    ...
    def compute_approx_vram_consumption(
        ...
        tmp = num_blocks_per_stage_encoder[0] * np.prod(
            current_shape
...
class PlainConvUNet(SegmentationNetwork):
    ...
    def compute_approx_vram_consumption(
        ...
        enc = PlainConvUNetEncoder.compute_approx_vram_consumption(
            patch_size,
            base_num_features,
            max_num_features,
            num_modalities,
    ...
    def compute_reference_for_vram_consumption_3d():
        ...
        return PlainConvUNet.compute_approx_vram_consumption(
            patch_size,
            32,
            512,
            4,
            3,
            pool_op_kernel_sizes,
            conv_per_stage_encoder,
            conv_per_stage_decoder,
            2,
            2,
        )
...
if __name__ == "__main__":
    ...
    dummy_gt = (
        (torch.rand((batch_size, 1, *patch_size)) * 4)
        .round()
        .clamp_(0, 3)
        .cuda()
        .long()
