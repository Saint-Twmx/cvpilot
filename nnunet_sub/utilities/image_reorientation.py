...
def revert_reorientation(image: str) -> None:
    ...
    assert isfile(expected_pkl), (
        "Must have a file with the original affine, as created by "
        "reorient_to_ras. Expected filename: %s" % expected_pkl
    )
    ...
    assert np.all(np.isclose(original_affine, restored_affine)), (
        "restored affine does not match original affine, " "aborting!"
    )
    nib.save(img, image)
