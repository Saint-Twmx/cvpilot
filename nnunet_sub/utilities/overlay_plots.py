...
from nnunet_sub.utilities.task_name_id_conversion import (
...
def generate_overlay(
    ...
    if len(image.shape) == 2:
        ...
        assert image.shape[2] == 3, (
            "if 3d image is given the last dimension must be the color channels "
...
def plot_overlay_preprocessed(
    ...
    assert modality_index < (
        data.shape[0] - 1
    ), "This dataset only supports modality index up to %d" % (data.shape[0] - 2)
...
def generate_overlays_for_task(
    task_name_or_id,
    output_folder,
    num_processes=8,
    modality_idx=0,
    ...
    if not use_preprocessed:
        ...
        identifiers = [
            i[:-7]
            for i in subfiles(join(folder, "labelsTr"), suffix=".nii.gz", join=False)
        ]
        ...
        multiprocessing_plot_overlay(
            image_files, seg_files, output_files, 0.6, num_processes
        )
    else:
...
def entry_point_generate_overlay():
    ...
    parser.add_argument(
        ...
        default=8,
        required=False,
        help="number of processes used. Default: 8",
    )
    parser.add_argument(
        "-modality_idx",
        type=int,
