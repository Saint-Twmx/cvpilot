...
from nnunet_sub.utilities.sitk_stuff import copy_geometry
from scipy.ndimage import label
from skimage.measure import regionprops


def load_remove_save_alg(
    img_npy: np.ndarray,
...
def load_remove_save(
    ...
    print(f"~~~~~~~~~~~~spacing: {img_in.GetSpacing()}")
    if "lowres" not in output_file:
        save_dir = "/ssd0/yx/save_results/test_seg3"
        my_lowres_seg_dir = os.path.join(save_dir, "ori/cascade")
    else:
        save_dir = "/ssd0/yx/save_results/test_seg3"
        my_lowres_seg_dir = os.path.join(save_dir, "ori/lowres")

    if not os.path.exists(my_lowres_seg_dir):
        os.makedirs(my_lowres_seg_dir)

    input_save_path = os.path.join(my_lowres_seg_dir, "postprocess_input.npy")
    np.save(input_save_path, img_npy)

    image, largest_removed, kept_size = remove_all_but_the_largest_connected_component(
        img_npy, for_which_classes, volume_per_voxel, minimum_valid_object_size
    )

    final_softmax_res_save_path = os.path.join(
        my_lowres_seg_dir, "final_out_postprocess.npy"
    )
    np.save(final_softmax_res_save_path, image)
    print(f"---------------save {final_softmax_res_save_path} success")
...
def remove_all_but_the_largest_connected_component(
    ...
    for c in for_which_classes:
        ...
        if num_objects > 0:
            ...
            maximum_size = max([obj.area for obj in objs])
            kept_size[c] = maximum_size

            for object_id in range(num_objects):
                # we only remove objects that are not the largest
                if objs[object_id].area != maximum_size:
                    # we only remove objects that are smaller than minimum_valid_object_size
                    remove = True
                    if minimum_valid_object_size is not None:
                        remove = objs[object_id].area < minimum_valid_object_size[c]
                    if remove:
                        image[
                            objs[object_id].coords[:, 0],
                            objs[object_id].coords[:, 1],
                            objs[object_id].coords[:, 2],
                        ] = 0
                        if largest_removed[c] is None:
                            largest_removed[c] = objs[object_id].area
                        else:
