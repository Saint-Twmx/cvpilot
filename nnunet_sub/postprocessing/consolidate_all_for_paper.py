...
def get_datasets():
    configurations_all = {
        ...
        "Task35_ISBILesionSegmentation": (
            "3d_fullres",
            "2d",
        ),
        "Task38_CHAOS_Task_3_5_Variant2": (
            "3d_fullres",
            "2d",
        ),
        "Task29_LITS": (
        ...
        "Task17_AbdominalOrganSegmentation": (
            "3d_cascade_fullres",
            "3d_lowres",
            "2d",
            "3d_fullres",
...
def get_commands(
    ...
    node_pool = (
        ["hdf18-gpu%02.0d" % i for i in range(1, 21)]
    ...
    for task in configurations:
        ...
        for m in models:
            ...
            folder = get_output_folder_name(
                ...
                trainer,
                plans,
                overwrite_training_output_dir="/datasets/datasets_fabian/results/nnUNet",
            )
            node = node_pool[ctr % len(node_pool)]
            print(
                'bsub -m %s -q gputest -L /bin/bash "source ~/.bashrc && python postprocessing/'
                "consolidate_postprocessing.py -f" % node,
                folder,
                '"',
            )
            ctr += 1
