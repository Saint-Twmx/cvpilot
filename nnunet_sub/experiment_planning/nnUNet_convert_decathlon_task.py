...
from nnunet_sub.experiment_planning.utils import split_4d
from nnunet_sub.utilities.file_endings import remove_trailing_slash


def crawl_and_remove_hidden_from_decathlon(folder):
    folder = remove_trailing_slash(folder)
    assert folder.split("/")[-1].startswith("Task"), (
        "This does not seem to be a decathlon folder. Please give me a "
        "folder that starts with TaskXX and has the subfolders imagesTr, "
        "labelsTr and imagesTs"
    )
    subf = subfolders(folder, join=False)
    assert "imagesTr" in subf, (
        "This does not seem to be a decathlon folder. Please give me a "
        "folder that starts with TaskXX and has the subfolders imagesTr, "
        "labelsTr and imagesTs"
    )
    assert "imagesTs" in subf, (
        "This does not seem to be a decathlon folder. Please give me a "
        "folder that starts with TaskXX and has the subfolders imagesTr, "
        "labelsTr and imagesTs"
    )
    assert "labelsTr" in subf, (
        "This does not seem to be a decathlon folder. Please give me a "
        "folder that starts with TaskXX and has the subfolders imagesTr, "
        "labelsTr and imagesTs"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="The MSD provides data as 4D Niftis with the modality being the first"
