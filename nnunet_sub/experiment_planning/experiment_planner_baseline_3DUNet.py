...
from nnunet_sub.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from nnunet_sub.experiment_planning.common_utils import (
    get_pool_and_conv_props_poolLateV2,
)
from nnunet_sub.experiment_planning.utils import create_lists_from_splitted_dataset
from nnunet_sub.network_architecture.generic_UNet import Generic_UNet
from nnunet_sub.paths import *
from nnunet_sub.preprocessing.cropping import get_case_identifier_from_npz
from nnunet_sub.training.model_restore import recursive_find_python_class
...
class ExperimentPlanner(object):
    ...
    def plan_experiment(self):
        ...
        if (
            np.prod(median_shape) / architecture_input_voxels_here
            < self.how_much_of_a_patient_must_the_network_see_at_stage0
        ):
            more = False
        else:
            ...
            while (
                ...
                * architecture_input_voxels_here
                ...
                if np.any((max_spacing / lowres_stage_spacing) > 2):
                    lowres_stage_spacing[
                        (max_spacing / lowres_stage_spacing) > 2
                    ] *= 1.01
                else:
                ...
                new = self.get_properties_for_stage(
                ...
                architecture_input_voxels_here = np.prod(
                    new["patch_size"], dtype=np.int64
                )
            if 2 * np.prod(
                new["median_patient_size_in_voxels"], dtype=np.int64
            ) < np.prod(
    ...
    def run_preprocessing(self, num_threads):
        ...
        preprocessor_class = recursive_find_python_class(
            [join(nnunet_sub.__path__[0], "preprocessing")],
            self.preprocessor_name,
            current_module="nnunet_sub.preprocessing",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-tl", type=int, required=False, default=8, help="num_threads_lowres"
    )
    parser.add_argument(
