...
from nnunet_sub.experiment_planning.utils import crop
from nnunet_sub.paths import *
from nnunet_sub.preprocessing.sanity_checks import verify_dataset_integrity
from nnunet_sub.training.model_restore import recursive_find_python_class
from nnunet_sub.utilities.task_name_id_conversion import convert_id_to_task_name
...
def main():
    ...
    search_in = join(nnunet_sub.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class(
            [search_in], planner_name3d, current_module="nnunet_sub.experiment_planning"
        )
        if planner_3d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere in "
                "nnunet_sub.experiment_planning" % planner_name3d
            )
    else:
        planner_3d = None
    ...
    if planner_name2d is not None:
        planner_2d = recursive_find_python_class(
            [search_in], planner_name2d, current_module="nnunet_sub.experiment_planning"
        )
        if planner_2d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere in "
                "nnunet_sub.experiment_planning" % planner_name2d
            )
    else:
        planner_2d = None
