...
def write_plans_to_file(f, plans_file):
    ...
    for stage in stages:
        patch_size_in_mm = [
            i * j
            for i, j in zip(
                a["plans_per_stage"][stages[stage]]["patch_size"],
        ...
        median_patient_size_in_mm = [
            i * j
            for i, j in zip(
                a["plans_per_stage"][stages[stage]]["median_patient_size_in_voxels"],
                a["plans_per_stage"][stages[stage]]["current_spacing"],
            )
        ]
        ...
        f.write(
            ";%s"
            % str(a["plans_per_stage"][stages[stage]]["median_patient_size_in_voxels"])
        )
        ...
        f.write(
            ...
            % str(
                [
                    str("%03.2f" % i)
                    for i in a["plans_per_stage"][stages[stage]]["current_spacing"]
                ]
            )
        )
        f.write(
            ...
            % str(
                [
                    str("%03.2f" % i)
                    for i in a["plans_per_stage"][stages[stage]]["original_spacing"]
                ]
            )
        )
        f.write(
            ";%s" % str(a["plans_per_stage"][stages[stage]]["pool_op_kernel_sizes"])
        )
...
if __name__ == "__main__":
    ...
    with open("2019_02_06_plans_summary.csv", "w") as f:
        f.write(
            "task;plans_file;stage;batch_size;num_pool_per_axis;patch_size;patch_size(mm);median_patient_size_in_voxels;median_patient_size_in_mm;current_spacing;original_spacing;pool_op_kernel_sizes;conv_kernel_sizes\n"
        )
        for t in task_dirs:
            print(t)
            tmp = join(base_dir, t)
            plans_files = [
                i
                for i in subfiles(tmp, suffix=".pkl", join=False)
                if i.find("_plans_") != -1 and i.find("Dgx2") == -1
            ]
