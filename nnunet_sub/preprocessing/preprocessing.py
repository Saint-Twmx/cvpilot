...
from nnunet_sub.configuration import (
    default_num_threads,
    RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD,
)
from nnunet_sub.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper
...
def resample_patient(
    ...
    new_shape = np.round(
        ((np.array(original_spacing) / np.array(target_spacing)).astype(float) * shape)
    ).astype(int)
    ...
    if data is not None:
        data_reshaped = resample_data_or_seg_alg(
            data,
            new_shape,
            False,
            axis,
        ...
        data_reshaped = None
    if seg is not None:
        seg_reshaped = resample_data_or_seg_alg(
...
def _normalize_spacing_torch(
    ...
    if tensor_new.dtype != torch.float64:
        tensor_new = tensor_new.to(torch.float64)
...
def resize_segmentation(segmentation, new_shape, order=3):
    ...
    if order == 0:
        ...
        return normalize_spacing(segmentation, new_shape, order=order)
    else:
        ...
        for i, c in enumerate(unique_labels):
            ...
            reshaped_multihot = normalize_spacing(mask, new_shape, order=order)
            reshaped[reshaped_multihot >= 0.5] = c
        return reshaped


def resize_segmentation_ori(segmentation, new_shape, order=3):
    """
    Resizes a segmentation map. Supports all orders (see skimage documentation). Will transform segmentation map to one
    hot encoding which is resized and transformed back to a segmentation map.
    This prevents interpolation artifacts ([0, 0, 2] -> [0, 1, 2])
    ...
    if order == 0:
        ...
        for i, c in enumerate(unique_labels):
            ...
            if not isinstance(new_shape, np.ndarray):
                new_shape = np.array(new_shape)
            reshaped_multihot = normalize_spacing(
                mask.astype(float), new_shape, order=3
            )
            reshaped[reshaped_multihot >= 0.5] = c
        return reshaped


def resample_data_or_seg_alg(
    ...
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
                    ...
                    if axis == 0:
                        if is_seg:
                            reshaped_data.append(
                                resize_segmentation(
                                    data[c, slice_id], new_shape_2d, 0, **kwargs
                                ).astype(dtype_data)
                            )
                        else:
                            reshaped_data.append(
                                normalize_spacing(
                                    data[c, slice_id], new_shape_2d, order=3
                                )
                            )
                    elif axis == 1:
                        if is_seg:
            ...
            for c in range(data.shape[0]):
                if is_seg:
                    reshaped.append(
                        resize_segmentation(data[c], new_shape, 0, **kwargs)[
                            None
                        ].astype(dtype_data)
                    )
                else:
                    reshaped.append(
                        normalize_spacing(data[c], new_shape, order=3)[None].astype(
                            dtype_data
                        )
                    )
...
def resample_data_or_seg(
    ...
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                reshaped.append(
                    resize_fn(data[c], new_shape, order, **kwargs)[None].astype(
                        dtype_data
                    )
                )
            reshaped_final_data = np.vstack(reshaped)
...
class GenericPreprocessor(object):
    def __init__(
        ...
        self.resample_separate_z_anisotropy_threshold = (
            RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD
        )
    ...
    def load_cropped(cropped_output_dir, case_identifier):
        ...
        seg = all_data[-1:]
        with open(
            os.path.join(cropped_output_dir, "%s.pkl" % case_identifier), "rb"
        ) as f:
    ...
    def resample_and_normalize(
        ...
        assert len(self.normalization_scheme_per_modality) == len(data), (
            "self.normalization_scheme_per_modality "
    ...
    def preprocess_test_case(
        ...
        data, seg, properties = ImageCropper.crop_from_file(data_files, seg_file)
    ...
    def _run_internal(
        ...
        with open(
            os.path.join(output_folder_stage, "%s.pkl" % case_identifier), "wb"
        ) as f:
    ...
    def run(
        ...
        all_classes = load_pickle(
            join(input_folder_with_cropped_npz, "dataset_properties.pkl")
        )["all_classes"]
...
class Preprocessor3DBetterResampling(GenericPreprocessor):
    ...
    def resample_and_normalize(
        self, data, target_spacing, properties, seg=None, force_separate_z=False
    ):
        """
        data and seg must already have been transposed by transpose_forward. properties are the un-transposed values
        (spacing etc)
        ...
        if force_separate_z is not False:
            print(
                "WARNING: Preprocessor3DBetterResampling always uses force_separate_z=False. "
...
class PreprocessorFor2D_edgeLength512(PreprocessorFor2D):
    ...
    def resample_and_normalize(
        ...
        after = {"spacing": "None", "data.shape (data is resampled)": data.shape}
        print("before:", before, "\nafter: ", after, "\n")
