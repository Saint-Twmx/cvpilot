...
import numpy as np
import torch
from nnunet_sub.training.loss_functions.TopK_loss import TopKLoss
...
class SoftDiceLoss(nn.Module):
    def __init__(
        ...
        self.apply_nonlin = apply_nonlin
        self.smooth = smooth
        self.classes_dc_weight = class_dc_weight
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if not self.do_bg:
            if self.batch_dice:
                ...
                dc = dc[:, 1:]
        if self.classes_dc_weight is not None:
            dc = dc * self.classes_dc_weight
        dc = dc.mean()
