...
from nnunet_sub.inference.segmentation_export import (
...
from nnunet_sub.training.data_augmentation.data_augmentation_moreDA import (
    get_moreDA_augmentation,
)
from nnunet_sub.training.dataloading.dataset_loading import unpack_dataset
from nnunet_sub.training.loss_functions.crossentropy import RobustCrossEntropyLoss
from nnunet_sub.training.loss_functions.dice_loss import get_tp_fp_fn_tn
from nnunet_sub.training.network_training.nnUNetTrainerV2 import nnUNetTrainerV2
from nnunet_sub.utilities.distributed import awesome_allgather_function
from nnunet_sub.utilities.nd_softmax import softmax_helper
from nnunet_sub.utilities.tensor_utilities import sum_tensor
from nnunet_sub.utilities.to_torch import to_cuda, maybe_to_torch
...
class nnUNetTrainerV2_DDP(nnUNetTrainerV2):
    def __init__(
        ...
        self.global_batch_size = (
            None  # we need to know this to properly steer oversample
        )
        ...
        for rank in range(world_size):
            ...
            if sample_id_high / self.global_batch_size < (
                ...
                percent_covered_by_this_rank = (
                    sample_id_high / self.global_batch_size
                    - sample_id_low / self.global_batch_size
                )
    ...
    def validate(
        ...
        ), "must initialize, ideally with checkpoint (or train first)"
        ...
        for k in my_keys:
            ...
            if k in my_keys:
                if (
                    ...
                    softmax_pred = (
                        self.predict_preprocessed_data_return_seg_and_softmax(
                            data[:-1],
                            do_mirroring=do_mirroring,
