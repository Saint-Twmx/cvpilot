...
from nnunet_sub.training.data_augmentation.custom_transforms import (
    Convert3DTo2DTransform,
    Convert2DTo3DTransform,
    MaskTransform,
    ConvertSegmentationToRegionsTransform,
)
from nnunet_sub.training.data_augmentation.default_data_augmentation import (
    get_patch_size,
)
from nnunet_sub.training.data_augmentation.downsampling import (
    DownsampleSegForDSTransform2,
)
from nnunet_sub.training.data_augmentation.pyramid_augmentations import (
...
from nnunet_sub.utilities.set_n_proc_DA import get_allowed_n_proc_DA
...
class nnUNetTrainerV2_DA5(nnUNetTrainerV2):
    ...
    def get_train_transforms(self) -> List[AbstractTransform]:
        ...
        tr_transforms.append(
            BlankRectangleTransform(
                [[max(1, p // 10), p // 3] for p in self.patch_size],
        ...
        if any(self.use_mask_for_norm.values()):
            tr_transforms.append(
                MaskTransform(
                    self.use_mask_for_norm, mask_idx_in_seg=0, set_outside_to=0
                )
            )
        ...
        if self.regions is not None:
            tr_transforms.append(
                ConvertSegmentationToRegionsTransform(self.regions, "target", "target")
            )
    ...
    def wrap_transforms(
        self, dataloader_train, dataloader_val, train_transforms, val_transforms
    ):
        tr_gen = NonDetMultiThreadedAugmenter(
            dataloader_train,
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            if training:
                ...
                self.tr_gen, self.val_gen = self.wrap_transforms(
                    self.dl_tr, self.dl_val, tr_transforms, val_transforms
                )
