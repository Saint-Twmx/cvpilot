...
class nnUNetTrainerV2_fullEvals(nnUNetTrainerV2):
    ...
    def __init__(
        ...
        self.num_val_batches_per_epoch = (
            0  # we dont need this because this does not evaluate on full images
        )
    ...
    def validate(
        ...
        csv_file = np.loadtxt(
            join(output_folder, "summary.csv"), skiprows=1, dtype=str, delimiter=","
        )[:, 1:]
        ...
        self.print_to_log_file(
            "Running the validation took %f seconds"
    ...
    def on_epoch_end(self):
        ...
        if (self.epoch + 1) % self.validate_every == 0:
            whole, core, enhancing = self.validate(
                do_mirroring=False,
            ...
            self.print_to_log_file(
                "After epoch %d: whole %0.4f core %0.4f enhancing: %0.4f"
