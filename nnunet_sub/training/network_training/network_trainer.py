...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.dataset_tr = (
            self.dataset_val
        ) = None  # do not need to be used, they just appear if you are using the suggested load_dataset_and_do_split
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            # increase patience
            if (
                self.train_loss_MA + self.train_loss_MA_eps
                < self.best_MA_tr_loss_for_patience
            ):
                self.best_MA_tr_loss_for_patience = self.train_loss_MA
                self.best_epoch_based_on_MA_tr_loss = self.epoch
