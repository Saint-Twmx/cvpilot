...
from nnunet_sub.network_architecture.generic_UNet_DP import Generic_UNet_DP
...
from nnunet_sub.utilities.to_torch import maybe_to_torch, to_cuda
from nnunet_sub.network_architecture.initialization import InitWeights_He
...
from nnunet_sub.training.network_training.nnUNetTrainer import nnUNetTrainer
...
class nnUNetTrainerV2_DP(nnUNetTrainerV2):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        super(nnUNetTrainerV2_DP, self).__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage,
                                                unpack_data, deterministic, fp16, class_weights, loss_F)
        self.init_args = (plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                          deterministic, num_gpus, distribute_batch_size, fp16, class_weights, loss_F)
    ...
    def initialize_network(self):
        ...
        self.network = Generic_UNet_DP(self.num_input_channels, self.base_num_features, self.num_classes,
                                       len(self.net_num_pool_op_kernel_sizes), self.conv_per_stage, 2, conv_op,
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if run_online_evaluation:
            self.run_online_evaluation_only(output[0], target[0])
        del target

        return l.detach().cpu().numpy()

    def run_online_evaluation_only(self, output, target):
        with torch.no_grad():
