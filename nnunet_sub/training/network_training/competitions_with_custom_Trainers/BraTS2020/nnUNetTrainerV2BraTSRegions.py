...
from nnunet_sub.evaluation.region_based_evaluation import (
    evaluate_regions,
    get_brats_regions,
)
from nnunet_sub.network_architecture.generic_UNet import Generic_UNet
...
from nnunet_sub.training.loss_functions.dice_loss import (
    DC_and_BCE_loss,
    get_tp_fp_fn_tn,
    SoftDiceLoss,
)
from nnunet_sub.training.network_training.nnUNetTrainerV2 import nnUNetTrainerV2
from nnunet_sub.training.network_training.nnUNetTrainerV2_DDP import nnUNetTrainerV2_DDP
...
class nnUNetTrainerV2BraTSRegions(nnUNetTrainerV2):
    def __init__(
        ...
        self.loss = DC_and_BCE_loss(
            {}, {"batch_dice": False, "do_bg": True, "smooth": 0}
        )
...
class nnUNetTrainerV2BraTSRegions_DDP(nnUNetTrainerV2_DDP):
                ...
                if self.unpack_data:
                    if self.local_rank == 0:
                        ...
                        all_present = all(
                            [
                                isfile(
                                    join(self.folder_with_preprocessed_data, i + ".npy")
                                )
                                for i in case_ids
                            ]
                        )
    ...
    def run_iteration(
        ...
        raise NotImplementedError(
            "this class has not been changed to work with pytorch amp yet!"
        )
