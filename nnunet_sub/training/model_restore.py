...
from nnunet_sub.training.network_training.nnUNetTrainerV2_CascadeFullRes import (
    nnUNetTrainerV2CascadeFullRes,
)

# import logging
from nnunet_sub.training.network_training.nnUNetTrainerV2_DP import nnUNetTrainerV2_DP
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    nnunet_sub.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info["init"]
    name = info["name"]
    search_in = join(nnunet_sub.__path__[0], "training", "network_training")
    if name == "nnUNetTrainerV2_DDP":
        name = "nnUNetTrainerV2"
        init = (pkl_file, ) + init[1:2] + init[3:9] + init[10:11]

    tr = recursive_find_python_class(
        [search_in], name, current_module="nnunet_sub.training.network_training"
    )
    ...
    if tr is None:
        raise RuntimeError(
            "Could not find the model trainer specified in checkpoint in nnunet_sub.trainig.network_training. If it "
            "is not located there, please move it or change the code of restore_model. Your model "
            "trainer can be located in any directory within nnunet_sub.trainig.network_training (search is recursive)."
            "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s "
            % (checkpoint, name)
        )
    ...
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of nnunet_sub. Attempting to load it "
...
def load_model_and_checkpoint_files_infer(
    ...
    pkl_file = join(folds[0], "%s.model.pkl" % checkpoint_name)
    info = load_pickle(pkl_file)
    init = info["init"]
    if "3d_lowres" in folder:
        trainer = nnUNetTrainerV2_DP(*init)
    else:
        trainer = nnUNetTrainerV2CascadeFullRes(*init)
    if mixed_precision is not None:
        trainer.fp16 = mixed_precision
...
def load_model_and_checkpoint_files(
    ...
    trainer.initialize(False)
    all_best_model_files = [join(i, "%s.model" % checkpoint_name) for i in folds]
    # logger.info("using the following model files: ", all_best_model_files)
    all_params = [
        torch.load(i, map_location=torch.device("cpu")) for i in all_best_model_files
    ]
    return trainer, all_params
