...
from nnunet_sub.postprocessing.connected_components import (
    load_remove_save,
    load_postprocessing,
)
...
def predict_cases_fastest(
    ...
    for preprocessed in preprocessing:
        ...
        for i, p in enumerate(params):
            ...
            cur_softmax_save_path = os.path.join(
                my_lowres_seg_dir, "softmax_" + str(i) + ".npy"
            )
            cur_segout_save_path = os.path.join(
                my_lowres_seg_dir, "segout_" + str(i) + ".npy"
            )
            np.save(cur_softmax_save_path, res[1])
            np.save(cur_segout_save_path, res[0])
        ...
        seg_allres_save_path = os.path.join(my_lowres_seg_dir, "seg_allres.npy")
        np.save(seg_allres_save_path, seg)
