...
def merge_files(files, properties_files, out_file, override, store_npz):
    if override or not isfile(out_file):
        ...
        reg_class_orders = [
            p["regions_class_order"] if "regions_class_order" in p.keys() else None
            for p in props
        ]
        ...
        if not all([i is None for i in reg_class_orders]):
            ...
            for r in reg_class_orders[1:]:
                assert tmp == r, (
                    "If merging files with regions_class_order, the regions_class_orders of all "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This script will merge predictions (that were prdicted with the "
    ...
    parser.add_argument(
        ...
        "--folders",
        nargs="+",
        help="list of folders to merge. All folders must contain npz " "files",
        required=True,
    )
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        "-t",
        "--threads",
        help="number of threads used to saving niftis",
        required=False,
        default=2,
        type=int,
    )
    parser.add_argument(
        "-pp",
        "--postprocessing_file",
        help="path to the file where the postprocessing configuration "
    ...
    parser.add_argument(
        "--npz", action="store_true", required=False, help="stores npz and pkl"
    )
