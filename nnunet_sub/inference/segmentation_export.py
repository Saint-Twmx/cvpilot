...
from nnunet_sub.preprocessing.preprocessing import (
    get_lowres_axis,
    get_do_separate_z,
...
from nnunet_sub.preprocessing.preprocessing import resize_segmentation
...
def save_segmentation_nifti_from_softmax(
    ...
    if isinstance(segmentation_softmax, str):
        assert isfile(segmentation_softmax), (
            "If isinstance(segmentation_softmax, str) then "
    ...
    if np.any(
        [
            i != j
            for i, j in zip(
                np.array(current_shape[1:]), np.array(shape_original_after_cropping)
            )
        ]
    ):
    ...
    if bbox is not None:
        ...
        for c in range(3):
            bbox[c][1] = np.min(
                (
                    bbox[c][0] + seg_old_spacing.shape[c],
...
def save_segmentation_nifti(
    ...
    print(f"___________seg_old_size: {seg_old_size}")
    print(f"___________seg_old_size shape: {seg_old_size.shape}")

    if isinstance(out_fname, str):
        seg_resized_itk = sitk.GetImageFromArray(seg_old_size.astype(np.uint8))
    ...
    # final_softmax_res_save_path = os.path.join(my_lowres_seg_dir, 'final_softmaxout.npy')
    # np.save(final_softmax_res_save_path, seg_old_size.astype(np.uint8))
    # print(f"---------------save {final_softmax_res_save_path} success")

    if not verbose:
        sys.stdout = sys.__stdout__

    return seg_old_size.astype(np.uint8)
